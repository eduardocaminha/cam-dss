    ...
    cell: ({ row }) => {
      ...
      return (
        <span className="flex min-w-0 items-baseline gap-2">
          ...
          <span className="truncate text-sm font-[650] uppercase">
            {r.title}
          </span>
        </span>
      )
          ...
          <Table className="w-full table-fixed">
            <TableHeader>
              {table.getHeaderGroups().map((hg) => (
                <TableRow
                  key={hg.id}
                  className="border-(--cam-border) bg-(--cam-fg)/5 hover:bg-(--cam-fg)/5"
                >
            ...
            <TableFooter className="border-t-2 border-(--cam-border)">
