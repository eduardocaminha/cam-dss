      ...
      <div className="relative flex flex-1 flex-col justify-center gap-5">
        ...
        {commandsFlow.map((entry) => (
          <div key={entry.command} className="flex items-baseline gap-4">
            ...
            />
            {/* Keycap: 1px frame with a doubled bottom edge, the terminal-key
                idiom — these are commands you type, not buttons you click. */}
            <kbd className="cam-mono kbd-key shrink-0 text-[13px] text-(--cam-fg)">
              {entry.command}
            </kbd>
