...
function Tag({ children }: { children: React.ReactNode }) {
  return (
    <span className="cam-label shrink-0 border border-(--cam-line) px-1.5 py-0.5 text-[11px] text-(--cam-fg-muted)">
      {children}
    </span>
  )
}
      ...
      <div className="flex flex-1 flex-col">
        {backlog.map((entry) => {
          ...
          return (
            <div
              key={entry.id}
              className="flex flex-col gap-1.5 border-b border-(--cam-line) py-3 last:border-b-0"
            >
